...
import { MARKETS as FALLBACK_MARKETS } from "@/data/markets";

import { GlassCard } from "@/components/ui/GlassCard";
import { SectionHeading } from "@/components/ui/SectionHeading";

const Globe = lazy(() =>
  import("@/components/globe/Globe").then((m) => ({
    default: m.Globe,
  }))
);
...
export function GlobalGlobeSection() {
  ...
  const {
    ...
    loading,
    error,
  } = useApi(() => api.markets(), FALLBACK_MARKETS);
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) =>
        entries.forEach(
          (e) => e.isIntersecting && setMount(true)
        ),
      { rootMargin: "200px" }
    );
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        {loading && (
          <div className="mt-6 text-center text-sm text-muted-foreground animate-pulse">
            Fetching global market intelligence...
          </div>
        )}
        ...
        <div className="relative mt-12 grid gap-6 lg:grid-cols-[1.6fr_1fr]">
          ...
          <div className="relative h-[520px] w-full overflow-hidden rounded-3xl border border-white/10 bg-[radial-gradient(ellipse_at_center,oklch(0.25_0.12_280),oklch(0.12_0.06_280))] md:h-[640px] glow-primary">
            <div
              aria-hidden
              className="pointer-events-none absolute inset-0 ring-grid opacity-20"
            />
            ...
            <div
              aria-hidden
              className="pointer-events-none absolute inset-y-0 left-0 w-px bg-gradient-to-b from-transparent via-white/20 to-transparent"
            />
            ...
            <div className="absolute left-4 top-4 z-10 flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
              ...
              Live · Global Markets · drag to rotate
            </div>
